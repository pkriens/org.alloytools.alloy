...
public interface Result<V> {
    ...
    /**
     * Returns an {@link Ok} if the {@code value} parameter is non-{@code null} or
     * an {@link Err} otherwise. At least one of {@code value} or {@code error} must
     * be non-{@code null}.
     *
     * @param <V> The value type of the Result.
     * @param value If {@code value} is non-{@code null}, an {@link Ok} result is
     *            returned with the specified value.
     * @param error If {@code value} is {@code null}, an {@link Err} result is
     *            returned with the specified error.
     * @return An {@link Ok} if the {@code value} parameter is non-{@code null} or
     *         an {@link Err} otherwise.
     */
    ...
    /**
     * Returns the contained value if this is an {@link Ok} value. Otherwise throws
     * a {@link ResultException}.
     *
    ...
    /**
     * Express the expectation that this object is an {@link Ok} value. Otherwise
     * throws a {@link ResultException} with the specified message.
     *
     * @param message The message to pass to a potential ResultException. Must not
     *            be {@code null}.
     * @throws ResultException If this is an {@link Err} instance.
     */
    ...
    /**
     * Returns the contained value if this is an {@link Ok} value. Otherwise returns
     * the specified alternate value.
     *
    ...
    /**
     * Returns the contained value if this is an {@link Ok} value. Otherwise returns
     * the alternate value supplied by the specified supplier.
     *
     * @param orElseSupplier The supplier to supply an alternate value if this is an
     *            {@link Err} instance. Must not be {@code null}.
     * @return The contained value or the alternate value
     */
    ...
    /**
     * Returns the contained value if this is an {@link Ok} value. Otherwise throws
     * the exception supplied by the specified function.
     *
     * @param <R> The exception type.
     * @param throwableSupplier The supplier to supply an exception if this is an
     *            {@link Err} instance. Must not be {@code null}. The supplier must
     *            return a non-{@code null} result.
     * @return The contained value.
     * @throws R The exception returned by the throwableSupplier if this is an
     *             {@link Err} instance.
     */
    <R extends Throwable> V orElseThrow(Function< ? super String, ? extends R> throwableSupplier) throws R;
     ...
     * @param mapper The function to map the contained value into a new value. Must
     *            not be {@code null}.
     * @return A result containing the mapped value if this is an {@link Ok} value.
     *         Otherwise this.
     */
    <U> Result<U> map(Function< ? super V, ? extends U> mapper);

    /**
     * Map the contained error if this is an {@link Err} value. Otherwise return
     * this.
     *
     * @param mapper The function to map the contained error into a new error. Must
     *            not be {@code null}. The function must return a non-{@code null}
     *            error.
     * @return A result containing the mapped error if this is an {@link Err} value.
     *         Otherwise this.
     */
    Result<V> mapErr(Function< ? super String, ? extends CharSequence> mapper);

    /**
     * FlatMap the contained value if this is an {@link Ok} value. Otherwise return
     * this.
     *
     * @param <U> The new value type.
     * @param mapper The function to flatmap the contained value into a new result.
     *            Must not be {@code null}. The function must return a
     *            non-{@code null} result.
     * @return The flatmapped result if this is an {@link Ok} value. Otherwise this.
     */
    <U> Result<U> flatMap(Function< ? super V, ? extends Result< ? extends U>> mapper);

    /**
     * Recover the contained error if this is an {@link Err} value. Otherwise return
     * this.
     * <p>
     * To recover with a recovery value of {@code null}, the
     * {@link #recoverWith(Function)} method must be used. The specified function
     * for {@link #recoverWith(Function)} can return {@code Result.ok(null)} to
     * supply the desired {@code null} value.
     *
     * @param recover The function to recover the contained error into a new value.
     *            Must not be {@code null}.
     * @return A result containing the new non-{@code null} value if this is an
     *         {@link Err} value. Otherwise this if this is an {@link Ok} value or
     *         the recover function returned {@code null}.
     */
    Result<V> recover(Function< ? super String, ? extends V> recover);

    /**
     * Recover the contained error if this is an {@link Err} value. Otherwise return
     * this.
     *
     * @param recover The function to recover the contained error into a new result.
     *            Must not be {@code null}. The function must return a
     *            non-{@code null} value.
     * @return A result if this is an {@link Err} value. Otherwise this.
     */
    Result<V> recoverWith(Function< ? super String, ? extends Result< ? extends V>> recover);

    /**
     * Processes the result.
     *
     * @param ok The consumer called this result is an {@link Ok} value. Must not be
     *            {@code null}.
     * @param err The consumer called when this result is an {@link Err} value. Must
     *            not be {@code null}.
     */
    void accept(Consumer< ? super V> ok, Consumer< ? super String> err);
